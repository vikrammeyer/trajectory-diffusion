...
from trajdiff.diffusion import Unet1D, GaussianDiffusion1D, sample
...
def main():
    ...
    parser.add_argument('-cp', '--checkpoint_path', required=True)
    parser.add_argument('-sl', '--seq_len', default=None)
    ...
    output_folder = setup(args, "sample-diffusion.log", print_stdout=args.print_stdout)

    dataset = MultiAgentDataset(args.dataset_folder, cfg, args.percent_history)
    channels = 2

    seq_length = args.seq_len if args.seq_len is not None else dataset.traj_steps
